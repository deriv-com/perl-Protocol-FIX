...
subtest "simple case :: NoRoutingIDs" => sub {
    ...
    my $f_1 = Protocol::FIX::Field->new(216, 'RoutingType', 'INT', {
        1 => 'TARGET_FIRM',
        2 => 'TARGET_LIST',
        3 => 'BLOCK_FIRM',
        4 => 'BLOCK_LIST',
...
subtest "artificial :: DATA & LENGTH field combination" => sub {
    ...
    like exception { Protocol::FIX::Group->new($f_0, [$f_2 => 0]); },
        qr/The field type 'LENGTH' must appear before field SecureData/;
    my $g = Protocol::FIX::Group->new($f_0, [$f_1 => 0, $f_2 => 0]);
    ok $g;
