...
subtest "simple component 'CommissionData'" => sub {
    ...
    my $f_4 = Protocol::FIX::Field->new(12, 'FundRenewWaiv', 'CHAR', {
        Y => 'YES',
        N => 'NO',
    });
...
subtest "component (AttrbGrp) with group(NoInstrAttrib)" => sub {
    ...
    my $f_1 = Protocol::FIX::Field->new(871, 'InstrAttribType', 'INT', {
        1 => 'FLAT',
        2 => 'ZERO_COUPON',
    });
...
subtest "component (InstrumentExtension) with complex subcomponent (AttrbGrp)" => sub {
    ...
    my $of_1 = Protocol::FIX::Field->new(668, 'DeliveryForm', 'INT', {
        1 => 'BOOKENTRY',
        2 => 'BEARER',
    });
    my $of_2 = Protocol::FIX::Field->new(869, 'PctAtRisk', 'PERCENTAGE');
    my $c_outer = Protocol::FIX::Component->new('InstrumentExtension', [
    ...
    is humanize($c_outer->serialize([
        ...
        PctAtRisk     => '0.06',
        AttrbGrp      => [ NoInstrAttrib => [[InstrAttribType => 'FLAT', InstrAttribValue => 'abc']] ],
